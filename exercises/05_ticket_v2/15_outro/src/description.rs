...
pub enum TicketDescriptionParsingError {
...
impl TryFrom<String> for TicketDescription {
    type Error = TicketDescriptionParsingError;
...
impl TryFrom<&str> for TicketDescription {
    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        if value.is_empty() {
            return Err(TicketDescriptionParsingError::EmptyDescription);
        }
        if value.len() > 500 {
            return Err(TicketDescriptionParsingError::TooLongDescription);
        }
